...
def step_one_data_uploader():
    # 第一步：上传数据
    uploaded_files = st.file_uploader("上传你的数据文件", type=["csv"], key='upload_original_data',
    ...
    if uploaded_files:
        for uploaded_file in uploaded_files:
            if uploaded_file is not None:
                ...
                if uploaded_file.name.endswith('_格式化特征集.csv'):  # _格式化特征集
            ...
            for data_index, data_item in enumerate(read_data_list):
                ...
                if len(df_plot.columns) >= 1:
                    for index, col in enumerate(df_plot.columns):
                        ...
                        trend_target_path = os.path.join(trend_plot_path, f'{col}趋势图.png')
                        fig_trend.savefig(trend_target_path)
                        if data_index == 0 and index == 0:
                            st.pyplot(fig_trend)
                            st.success(f"趋势图已保存至: {trend_target_path}")
    with col2:
            ...
            for data_index, data_item in enumerate(read_data_list):
                ...
                if len(df_plot.columns) >= 1:
                    for index, col in enumerate(df_plot.columns):
                        ...
                        scatter_plot_path = os.path.join(result_folder, '可视化结果', f'{device_name}_可视化结果',
                                                         subfolders['scatter_dir']
                                                         )
                        os.makedirs(scatter_plot_path, exist_ok=True)
                        scatter_target_path = os.path.join(scatter_plot_path, f'{col}散点图.png')
                        fig_scatter.savefig(scatter_target_path)
                        plt.close()
                        if data_index == 0 and index == 0:
                            st.pyplot(fig_scatter)
                            st.success(f"散点图已保存至: {scatter_target_path}")
    with col3:
            ...
            for data_index, data_item in enumerate(read_data_list):
                ...
                if len(df_plot.columns) >= 1:
                    for index, col in enumerate(df_plot.columns):
                        ...
                        ax_hist.grid(True)
                        hist_plot_path = os.path.join(result_folder, '可视化结果', f'{device_name}_可视化结果',
                                                      subfolders['hist_dir']
                                                      )
                        os.makedirs(hist_plot_path, exist_ok=True)
                        hist_target_path = os.path.join(hist_plot_path, f'{col}直方图.png')
                        fig_hist.savefig(hist_target_path)
                        if data_index == 0 and index == 0:
                            st.pyplot(fig_hist)
            ...
            for data_index, data_item in enumerate(read_data_list):
                ...
                if len(df_plot.columns) >= 1:
                    for index, col in enumerate(df_plot.columns):
                        ...
                        ax_box.set_ylabel('值')
                        ax_box.grid(True)
                        box_plot_path = os.path.join(result_folder, '可视化结果', f'{device_name}_可视化结果',
                                                     subfolders['box_dir']
                                                     )
                        os.makedirs(box_plot_path, exist_ok=True)
                        box_target_path = os.path.join(box_plot_path, f'{col}直方图.png')
                        fig_box.savefig(box_target_path)
                        if data_index == 0 and index == 0:
                            st.pyplot(fig_box)
                            st.success(f"箱型图已保存至: {box_target_path}")
...
def step_four_condition_split(read_data_list, filtered_data_list,result_folder,subfolders):
    ...
    if st.checkbox('进行聚类分析', key='聚类操作'):
        ...
        for index,item in enumerate(cluster_data):
            ...
            if len(remaining_columns) > 0:
                # 选择聚类数目
                num_clusters = st.slider("选择聚类数量", 2, 10, 2,key=f'{device_name}_聚类')
                ...
                # 将聚类结果保存到文件
                clustered_file_path = os.path.join(result_folder,subfolders['cluster_dir'],f'{device_name}_聚类结果')
                os.makedirs(clustered_file_path, exist_ok=True)
                df.to_csv(os.path.join(clustered_file_path,'clustered_data.csv'), index=False)
                if index == 0:
                    # 显示聚类结果
                    st.subheader(f'聚类结果 (K={num_clusters})')
                    st.success(f"聚类结果已保存至: {os.path.join(clustered_file_path,'clustered_data.csv')}")
                ...
                cluster_plot_path = os.path.join(result_folder, subfolders['cluster_dir'],f'{device_name}_聚类结果')
                os.makedirs(cluster_plot_path, exist_ok=True)
                cluster_target_path=os.path.join(cluster_plot_path,'cluster_plot.png')
                fig_cluster.savefig(cluster_target_path)
                if index==0:
                    st.subheader('聚类散点图')
                    st.pyplot(fig_cluster)
                    st.success(f"聚类散点图已保存至: {cluster_target_path}")
            else:
                st.warning("没有足够的数值型数据进行聚类分析。")


def step_five_feature_generate(read_data_list, result_folder,data_folder):
    # 步骤5：特征生成
...
def step_six_feature_selection():
    ...
    second_uploaded_file = st.file_uploader(
        ...
        key=f'{device_name}_read_generated_features', accept_multiple_files=False)
...
def main():
    ...
    create_sidebar()
    # 步骤1: 上传文件
    read_data_list = step_one_data_uploader()

    if read_data_list is not None and len(read_data_list) != 0:
